...
import { productAPI, validatorAPI } from "@/lib/api";
import { Product, STATUS_COLORS, ROLE_LABELS } from "@/lib/types";
import {
  Package, Users, ShoppingBag, AlertTriangle,
  ArrowUpRight, Plus, UserCheck, BarChart3, Loader2
} from "lucide-react";
...
export function NGODashboard() {
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8">
      ...
      {/* Recent Products + Artisan Alerts */}
            ...
            {loading ? (
              ...
              <div className="space-y-2">
                {products.slice(0, 5).map(p => (
                  <div key={p.productId} className="flex items-center justify-between rounded-lg border border-white/5 bg-white/5 p-3 hover:bg-white/10 transition-colors cursor-pointer" onClick={() => router.push(`/products/${p.productId}`)}>
                    <div>
        ...
        <Card className="bg-gray-900/60 border-white/10">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 mt-4">
              <p className="text-xs text-amber-400 font-medium">💡 Tip</p>
              <p className="text-xs text-gray-400 mt-1">Only verified artisans can have products registered. Make sure artisans upload GI certificates.</p>
            </div>
          </CardContent>
        </Card>
      </div>
