...
const { encryptAadhaar, maskAadhaar } = require('../utils/crypto.util');

exports.register = async (req, res) => {
  try {
    const { name, email, password, role, org, location, aadhaar } = req.body;
    ...
    // Validate Aadhaar (12 digits, required)
    if (!aadhaar || !/^\d{12}$/.test(aadhaar)) {
      return res.status(400).json({ success: false, message: 'Valid 12-digit Aadhaar number is required for KYC' });
    }

    const existing = await User.findOne({ email });
    if (existing) return res.status(409).json({ success: false, error: 'AUTH_004', message: 'Email already registered' });

    // Encrypt Aadhaar before storing — plaintext NEVER saved
    const aadhaarEncrypted = encryptAadhaar(aadhaar);
    const aadhaarLastFour = aadhaar.slice(-4);

    const user = await User.create({
      name, email, passwordHash: password, role, org, location,
      aadhaarEncrypted,
      aadhaarLastFour,
      kycVerified: true,  // Auto-verified on submission for hackathon
    });

    const token = jwt.sign(
      { id: user._id, name: user.name, email: user.email, role: user.role, org: user.org },
